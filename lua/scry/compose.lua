  ...
  require("conjurer").get_provider()({
    ...
    vim.schedule(function()
      ...
      if #res.changed > 0 then
        parts[#parts + 1] = ("%d changed"):format(#res.changed)
      end
      if #res.created > 0 then
        parts[#parts + 1] = ("%d created"):format(#res.created)
      end
      if #res.refused > 0 then
        parts[#parts + 1] = ("%d refused (not this feature's)"):format(#res.refused)
      end
      vim.notify(
        ("[scry] %s — %s, in buffers and UNSAVED: `:w` what you keep, `:e!` what you don't"):format(
          feature.name,
          table.concat(parts, ", ")
        )
      )
